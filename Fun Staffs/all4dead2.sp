...
public OnPluginStart() {
	...
	notify_players = CreateConVar("a4d_notify_players", "0", "Whether or not we announce changes in game.", FCVAR_PLUGIN);	
	zombies_increment = CreateConVar("a4d_zombies_to_add", "10", "The amount ofzombies to add when an admin requests morezombies.", FCVAR_PLUGIN, true, 10.0, true, 100.0);
 ...
 * 	is to fix an issue where entities created through z_spawn_old have random amount 
 * 	of health
 * </summary>
 * <remarks>
...
public Action:Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (GetClientTeam(client) == 3 && currently_spawning) {
		...
		LogAction(0, -1, "[提醒] Given full health to client %L that (hopefully) was spawned by A4D.", client);
...
public OnEntityCreated(entity, const String:classname[]) {
	// If the last thing that was spawned as a僵尸 then store that entity
	// for future use
...
public Action:Menu_CreateSpecialInfectedMenu(client, args) {
	...
	SetMenuTitle(menu, "创建特殊感染者");
	SetMenuExitBackButton(menu, true);
	SetMenuExitButton(menu, true);
	if (GetConVarBool(automatic_placement))
		AddMenuItem(menu, "ap", "关闭自动放置位置");
	else 
		AddMenuItem(menu, "ap", "开启自动放置位置");
...
public Action:Menu_CreateUInfectedMenu(client, args) {
	...
	if (GetConVarBool(automatic_placement))
		...
		AddMenuItem(menu, "ap", "开启自动放置位置");
	AddMenuItem(menu, "s1", "创建城管僵尸");
	AddMenuItem(menu, "s2", "创建CEDA僵尸");
	AddMenuItem(menu, "s3", "创建小丑僵尸");
	AddMenuItem(menu, "s4", "创建泥人僵尸");
	AddMenuItem(menu, "s5", "创建工人僵尸");
	AddMenuItem(menu, "s6", "创建车手僵尸");
	AddMenuItem(menu, "s7", "创建堕落生还者僵尸");
	DisplayMenu(menu, client, MENU_DISPLAY_TIME);
	return Plugin_Handled;
}
/// Handles callbacks from a client using the spawning menu.
...
public Action:Command_SpawnInfected(client, args) { 
	if (args < 1) {
		ReplyToCommand(client, "使用: a4d_spawn_infected <infected_type> (无法用于非特感, 使用 a4d_spawn_uinfected 代替)"); 
	} else {
		new String:type[16];
...
public Action:Command_SpawnUInfected(client, args) { 
	if (args < 1) {
		ReplyToCommand(client, "使用: a4d_spawn_uinfected <riot|ceda|clown|mud|roadcrew|jimmy>"); 
	} else {
		new String:type[32];
		GetCmdArg(1, type, sizeof(type));
		new number;
...
/**
 * <summary>
 * 	Spawns one of the specified infected using the z_spawn_old command. 
 * </summary>
 * <param name="type">
...
public Action:Command_EnableAutoPlacement(client, args) {
	if (args < 1) {
		ReplyToCommand(client, "使用: a4d_enable_auto_placement <0|1>");
		return Plugin_Handled;
	}
	new String:value[16];
 ...
 * 	it will not look like they are magically appearing. This only affectszombies
 * 	spawned through z_spawn_old.
 * </remarks>
*/
Do_EnableAutoPlacement(client, bool:value) {
	SetConVarBool(automatic_placement, value);
	if (value == true)
		NotifyPlayers(client, "已开启自动放置僵尸位置.");
	else
		NotifyPlayers(client, "已关闭自动放置僵尸位置.");
	LogAction(client, -1, "(%L)设置 %s 为%i", client, "a4d_automatic_placement", value);	
}
...
public Action:Menu_CreateItemMenu(client, args) {
	...
	SetMenuExitBackButton(menu, true);
	SetMenuExitButton(menu, true);
	AddMenuItem(menu, "sd", "创建电击器");
	AddMenuItem(menu, "sm", "创建医疗包");
	AddMenuItem(menu, "sp", "创建药品");
	AddMenuItem(menu, "sa", "创建肾上腺素");
	AddMenuItem(menu, "sv", "创建燃烧弹");
	AddMenuItem(menu, "sb", "创建土制炸药");
	AddMenuItem(menu, "sb", "创建胆汁罐");
	AddMenuItem(menu, "sg", "创建汽油桶");
	AddMenuItem(menu, "st", "创建燃气罐");
	AddMenuItem(menu, "so", "创建氧气瓶");
	AddMenuItem(menu, "sa", "创建一堆弹药");
	AddMenuItem(menu, "si", "创建燃烧弹包");
	AddMenuItem(menu, "se", "创建高爆弹包");
	AddMenuItem(menu, "lp", "创建激光瞄准器包");
	AddMenuItem(menu, "gn", "创建矮人玩具(仅用于成就)");
	DisplayMenu(menu, client, MENU_DISPLAY_TIME);
	return Plugin_Handled;
}
...
public Action:Command_SpawnItem(client, args) { 
	if (args < 1) {
		ReplyToCommand(client, "使用: a4d_spawn_item <item_type>");
	} else {
		new String:type[16];
		GetCmdArg(1, type, sizeof(type));
...
Do_SpawnItem(client, const String:type[]) {
	new String:feedback[64], String:playername[128];
	Format(feedback, sizeof(feedback), "已成功创建了 %s", type);
	GetClientName(client, playername, sizeof(playername));
	new AdminId:id = GetUserAdmin(client);
	if (client == 0 || !GetAdminFlag(id, Admin_Root)) {
		ReplyToCommand(client, "\x03该命令没有足够的权限使用\x01 \n\n"); 
		PrintToChatAll("\x04%N\x01 试图使用作弊命令给予自己 \x05%s\x04 已被系统检测并阻止\x01,请各位以此为戒", playername, type);
	} else {
		StripAndExecuteClientCommand(client, "give", type);
		NotifyPlayers(client, feedback);
		LogAction(client, -1, "[提醒]: (%L) 创建了 %s", client, type);
	}
}
...
Do_CreateEntity(client, const String:name[], const String:model[], Float:location[3], const bool:zombie) {
	...
	LogAction(client, -1, "[提醒]: (%L) has created a %s (%s)", client, name, model);
}
...
public Action:Menu_CreateWeaponMenu(client, args) {
	...
	SetMenuExitBackButton(menu, true);
	SetMenuExitButton(menu, true);
	AddMenuItem(menu, "sp", "创建双手枪");
	AddMenuItem(menu, "sg", "创建沙鹰");
	AddMenuItem(menu, "ss", "创建散弹枪");
	AddMenuItem(menu, "sa", "创建连喷");
	AddMenuItem(menu, "ssa", "创建SPAS12");
	AddMenuItem(menu, "sm", "创建冲锋枪");
	AddMenuItem(menu, "s3", "创建消音器冲锋枪");
	AddMenuItem(menu, "sr", "创建M16步枪");
	AddMenuItem(menu, "s1", "创建AK74");
	AddMenuItem(menu, "s2", "创建SCAR-Light步枪");
	AddMenuItem(menu, "sh", "创建猎枪");
	AddMenuItem(menu, "s4", "创建G3/SG1狙击步枪");
	AddMenuItem(menu, "s5", "创建榴弹炮");
	AddMenuItem(menu, "m6", "创建M60");
	DisplayMenu(menu, client, MENU_DISPLAY_TIME);
	return Plugin_Handled;
}
...
public Action:Menu_CreateMeleeWeaponMenu(client, args) {
	...
	SetMenuExitBackButton(menu, true);
	SetMenuExitButton(menu, true);
	AddMenuItem(menu, "ma", "创建棒球棒");
	AddMenuItem(menu, "mb", "创建链锯");
	AddMenuItem(menu, "mc", "创建板球棒");
	AddMenuItem(menu, "md", "创建撬棍");
	AddMenuItem(menu, "me", "创建电吉他");
	AddMenuItem(menu, "mf", "创建消防斧");
	AddMenuItem(menu, "mg", "创建平底锅");
	AddMenuItem(menu, "mh", "创建太刀");
	AddMenuItem(menu, "mi", "创建砍刀");
	AddMenuItem(menu, "mj", "创建城管棍");
	AddMenuItem(menu, "mk", "创建军刀");
	DisplayMenu(menu, client, MENU_DISPLAY_TIME);
	return Plugin_Handled;
}
/// Handles callbacks from a client using the spawn weapon menu.
...
public Action:Menu_CreateDirectorMenu(client, args) {
	...
	SetMenuExitBackButton(menu, true);
	SetMenuExitButton(menu, true);
	AddMenuItem(menu, "fp", "开始一个事件");
	if (GetConVarBool(FindConVar("director_panic_forever"))) { AddMenuItem(menu, "pf", "[关闭]无限尸潮事件"); } else { AddMenuItem(menu, "pf", "[开始]无限尸潮事件"); }
	if (GetConVarBool(FindConVar("director_force_tank"))) { AddMenuItem(menu, "ft", "[开启]Tank出现在这一关卡"); } else { AddMenuItem(menu, "ft", "[关闭]Tank出现在这一关卡"); }
	if (GetConVarBool(FindConVar("director_force_witch"))) { AddMenuItem(menu, "fw", "[开启]Witch出现在这一关卡"); } else { AddMenuItem(menu, "fw", "[关闭]Witch出现在这一关卡"); }
	if (GetConVarBool(always_force_bosses)) { AddMenuItem(menu, "fd", "[开启]Boss自动出现在本关卡"); } else { AddMenuItem(menu, "fw", "[关闭]Boss自动出现在本关卡"); }
	AddMenuItem(menu, "mz", "增加更多一小团僵尸");	
	DisplayMenu(menu, client, MENU_DISPLAY_TIME);
	return Plugin_Handled;
}
...
public Action:Command_AlwaysForceBosses(client, args) {
	if (args < 1) { 
		ReplyToCommand(client, "使用: a4d_always_force_bosses <0|1>"); 
		return Plugin_Handled;
	}
	new String:value[2];
	GetCmdArg(1, value, sizeof(value));
...
Do_AlwaysForceBosses(client, bool:value) {
	...
	if (value == true)
		NotifyPlayers(client, "Boss 一定本关卡出现.");
	else
		NotifyPlayers(client, "Boss 一定不会在本关卡出现");
}
...
Do_ForcePanic(client) {
	...
	NotifyPlayers(client, "僵尸发起了进攻!");	
	LogAction(client, -1, "[提醒]: (%L) 执行了 %s", client, "a4d_force_panic");
}
/// Sourcemod Action for the Do_PanicForever command.
public Action:Command_PanicForever(client, args) {
	if (args < 1) { 
		ReplyToCommand(client, "使用: a4d_panic_forever <0|1>"); 
		return Plugin_Handled;
	}
	new String:value[2];
	GetCmdArg(1, value, sizeof(value));
...
Do_PanicForever(client, bool:value) {
	...
	if (value == true)
		NotifyPlayers(client, "无限的僵尸爆动开始了.");
	else
		NotifyPlayers(client, "无限的僵尸暴动停止了.");
}
/// Sourcemod Action for the Do_ForceTank command.
public Action:Command_ForceTank(client, args) {
	if (args < 1) { 
		ReplyToCommand(client, "使用: a4d_force_tank <0|1>"); 
		return Plugin_Handled; 
	}
...
Do_ForceTank(client, bool:value) {
	...
	if (value == true)
		NotifyPlayers(client, "Tank 一定会在本关卡出现");
	else
		NotifyPlayers(client, "Tank 一定不会在本关卡出现");
}
/// Sourcemod Action for the Do_ForceWitch command.
public Action:Command_ForceWitch(client, args) {
	if (args < 1) { 
		ReplyToCommand(client, "使用: a4d_force_witch <0|1>"); 
		return Plugin_Handled;
	}
	new String:value[2];
	GetCmdArg(1, value, sizeof(value));
...
Do_ForceWitch(client, bool:value) {
	...
	if (value == true)
		NotifyPlayers(client, "Witch 一定会在本关卡出现");	
	else 
		NotifyPlayers(client, "Witch 一定不会在本关卡出现");
}
...
public Action:Command_AddZombies(client, args) {
	if (args < 1) { 
		ReplyToCommand(client, "使用: a4d_add_zombies <0..99>"); 
		return Plugin_Handled;
	}
	new String:value[4];
	GetCmdArg(1, value, sizeof(value));
...
/**
 * <summary>
 * 	The director will spawn morezombies in the mobs and mega mobs.
 * </summary>
 * <remarks>
...
public Action:Menu_CreateConfigMenu(client, args) {
	...
	SetMenuTitle(menu, "配置选项");
	SetMenuExitBackButton(menu, true);
	SetMenuExitButton(menu, true);
	if (GetConVarBool(notify_players)) { AddMenuItem(menu, "pn", "[关闭]玩家提醒"); } else { AddMenuItem(menu, "pn", "[开启]玩家提醒"); }
	AddMenuItem(menu, "rs", "重设所有配置参数为默认值");
	DisplayMenu(menu, client, MENU_DISPLAY_TIME);
	return Plugin_Handled;
}
...
public Action:Command_EnableNotifications(client, args) {
	if (args < 1) { 
		ReplyToCommand (client, "使用: a4d_enable_notifications <0|1>"); 
		return Plugin_Handled;
	}
	new String:value[2];
	GetCmdArg(1, value, sizeof(value));
...
Do_EnableNotifications(client, bool:value) {
	...
	NotifyPlayers(client, "玩家提醒已开启.");
	LogAction(client, -1, "(%L)设置 %s 为%i", client, "a4d_notify_players", value);	
}
...
Do_ResetToDefaults(client) {
	...
	NotifyPlayers(client, "重设所有参数为默认值.");
	LogAction(client, -1, "(%L) 执行了 %s", client, "a4d_reset_to_defaults");
}

/// Sourcemod Action for the Do_EnableAllBotTeam command.
public Action:Command_EnableAllBotTeams(client, args) {
	if (args < 1) { 
		ReplyToCommand(client, "使用: a4d_enable_all_bot_teams <0|1>"); 
		return Plugin_Handled;
	}
...
Do_EnableAllBotTeam(client, bool:value) {
	...
	if (value == true)
		NotifyPlayers(client, "运行 BOT 队伍.");	
	else
		NotifyPlayers(client, "我们需要至少一个玩家以便于启动游戏.");
}
...
StripAndChangeServerConVarBool(client, String:command[], bool:value) {
	...
	LogAction(client, -1, "[提醒]: (%L)设置 %s 为%i", client, command, value);	
}
