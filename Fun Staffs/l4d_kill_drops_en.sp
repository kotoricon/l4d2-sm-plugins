...
public OnPluginStart()
{
	...
	l4d_loot_hunter = CreateConVar("l4d_loot_hunter", "10.0", "loot of hunter %[0-100]", FCVAR_PLUGIN);
	l4d_loot_smoker = CreateConVar("l4d_loot_smoker", "15.0", "loot of smoker %[0-100]", FCVAR_PLUGIN);
	l4d_loot_boomer = CreateConVar("l4d_loot_boomer", "15.0", "loot of Boomer %[0-100]", FCVAR_PLUGIN);
	l4d_loot_spitter = CreateConVar("l4d_loot_spitter", "10.0", "loot of spitter %[0-100]", FCVAR_PLUGIN);
	...
	l4d_loot_killtank_reward = CreateConVar("l4d_loot_killtank_reward", "0", "probability of reward for kill tank %[0-100]", FCVAR_PLUGIN);
	l4d_loot_killwitch_reward = CreateConVar("l4d_loot_killwitch_reward", "0", "probability of reward for kill witch %[0-100]", FCVAR_PLUGIN);
	...
	l4d_kill_addhp_mult = CreateConVar("l4d_kill_addhp_mult", "0.01", "health recover factor", FCVAR_PLUGIN);
	l4d_kill_addhp_lt50_mult = CreateConVar("l4d_kill_addhp_lt50_mult", "1.0", "health recover factor when health < 50", FCVAR_PLUGIN);
	l4d_kill_addhp_healthlimit = CreateConVar("l4d_kill_addhp_healthlimit", "100.0", "health limit", FCVAR_PLUGIN);

	l4d_kill_show_msg = CreateConVar("l4d_kill_show_msg", "0", "print kill messge 1:in chatŒ2: int hintbox, 3:all, 0:disable", FCVAR_PLUGIN);
	...
	l4d_kill_addhp_headshot_mult = CreateConVar("l4d_kill_addhp_headshot_mult", "1.0", "health recover multiple with headshot ", FCVAR_PLUGIN);

	l4d_kill_addhp_weapon_pistol_1 = CreateConVar("l4d_kill_addhp_weapon_pistol_1", "1.0", "health recover factor of pistol killing", FCVAR_PLUGIN);
	l4d_kill_addhp_weapon_msg_1 = CreateConVar("l4d_kill_addhp_weapon_msg_1", "1.0", "health recover factor of smg", FCVAR_PLUGIN);
	l4d_kill_addhp_weapon_rifle_1 = CreateConVar("l4d_kill_addhp_weapon_rifle_1", "1.0", "health recover factor of rifle killing", FCVAR_PLUGIN);
	l4d_kill_addhp_weapon_sniper_1 = CreateConVar("l4d_kill_addhp_weapon_sniper_1", "1.0", "health recover factor of sniper killing", FCVAR_PLUGIN);
	l4d_kill_addhp_weapon_shotgun_1 = CreateConVar("l4d_kill_addhp_weapon_shotgun_1", "1.0", "health recover factor of shotgun killing", FCVAR_PLUGIN);
	l4d_kill_addhp_weapon_melee = CreateConVar("l4d_kill_addhp_weapon_melee", "1.0", "health recover factor of melee killing", FCVAR_PLUGIN);
	...
	l4d_kill_addhp_weapon_pistol_2 = CreateConVar("l4d_kill_addhp_weapon_pistol_2", "0.0", "health recover factor of long range pistol killing", FCVAR_PLUGIN);
	l4d_kill_addhp_weapon_msg_2 = CreateConVar("l4d_kill_addhp_weapon_msg_2", "1.0", "health recover factor of long range smg killing", FCVAR_PLUGIN);
	l4d_kill_addhp_weapon_rifle_2 = CreateConVar("l4d_kill_addhp_weapon_rifle_2", "1.0", "health recover factor of long range rifle killing", FCVAR_PLUGIN);
	l4d_kill_addhp_weapon_sniper_2 = CreateConVar("l4d_kill_addhp_weapon_sniper_2", "1.0", "health recover factor of long range sniper killing", FCVAR_PLUGIN);
	l4d_kill_addhp_weapon_shotgun_2 = CreateConVar("l4d_kill_addhp_weapon_shotgun_2", "1.0", "health recover factor of long range shotgun killing", FCVAR_PLUGIN);
...
/////////////////////////////////////////////////////////////////////////
	...
	if (class == ZOMBIECLASS_TANK)
	{
		...
		else
		{
  		...
  		SpawnItemFromDieResult(victim, GetConVarFloat(l4d_loot_tank), GetConVarFloat(l4d_loot_tank_num),"");
		CreateTimer(5.0, BossDeadLaught);
 		if(ShowKillMsgOnChat())
		{
			 PrintToChatAll(chatmsg);
	...
	if(r < p)
	{
		...
		for(new i=0; i<num ; i++)
		{
			...
			else if(r<(a+b+c))
			{
				...
				if(r1==0) Give(client, "fireaxe");
				if(r1==1)	Give(client, "electric_guitar");

				if(r1==2) Give(client, "crowbar");
				if(r1==3) Give(client, "katana");
				if(r1==4) Give(client, "katana");
				if(r1==5) Give(client, "weapon_fireworkcrate");
				if(r1==6) Give(client, "golfclub");
				if(r1==7) Give(client, "frying_pan");
				if(r1==8) Give(client, "machete");
				if(r1==9) Give(client, "cricket_bat");
				if(r1==10) Give(client, "baseball_bat");
