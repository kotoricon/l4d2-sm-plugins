...
static Handle:SpecialAmmoMultiplier = INVALID_HANDLE;
static Handle:AdminSpecialAmmoMultiplier = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	AssaultAmmoCVAR = CreateConVar("l4d2_guncontrol_assaultammo", "600", " How much Ammo for Assault Rifles ", DEFAULT_FLAGS);
	SMGAmmoCVAR = CreateConVar("l4d2_guncontrol_smgammo", "900", " How much Ammo for SMG gun types ", DEFAULT_FLAGS);
	ShotgunAmmoCVAR = CreateConVar("l4d2_guncontrol_shotgunammo", "80", " How much Ammo for Shotgun and Chrome Shotgun ", DEFAULT_FLAGS);
	AutoShotgunAmmoCVAR = CreateConVar("l4d2_guncontrol_autoshotgunammo", "150", " How much Ammo for Autoshottie and SPAS ", DEFAULT_FLAGS);
	HRAmmoCVAR = CreateConVar("l4d2_guncontrol_huntingrifleammo", "300", " How much Ammo for the Hunting Rifle ", DEFAULT_FLAGS);
	SniperRifleAmmoCVAR = CreateConVar("l4d2_guncontrol_sniperrifleammo", "600", " How much Ammo for the Military Sniper Rifle, AWP, and Scout ", DEFAULT_FLAGS);	
	GrenadeLauncherAmmoCVAR = CreateConVar("l4d2_guncontrol_grenadelauncherammo", "60", " How much Ammo for the Grenade Launcher ", DEFAULT_FLAGS);
	M60AmmoCVAR = CreateConVar("l4d2_guncontrol_m60ammo", "254", " How much Ammo for the M60 ", DEFAULT_FLAGS);	
	
	GrenadeResupplyCVAR = CreateConVar("l4d2_guncontrol_allowgrenadereplenish", "0", " Do you allow Players to resupply the Grenadelauncher off ammospots ", DEFAULT_FLAGS);
	...
	SpecialAmmoMultiplier = CreateConVar("l4d2_guncontrol_specialammomulti", "2", " Multiplier for Special Ammo Pickup Amount ", DEFAULT_FLAGS);
	AdminSpecialAmmoMultiplier = CreateConVar("l4d2_guncontrol_adminspecialammomulti", "5", " Multiplier for Admin only Special Ammo Pickup Amount ", DEFAULT_FLAGS);
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	if (buttons & IN_USE && !buttondelay[client])
	{
		...
		if(id == INVALID_ADMIN_ID || !GetAdminFlag(id, Admin_Root) || GetAdminImmunityLevel(id) < 99) 
		{
			if (StrEqual(ent_name, "weapon_ammo_spawn", false) && StrEqual(currentgunname, "weapon_grenade_launcher", false) && GetConVarBool(GrenadeResupplyCVAR))
			{
				...
				PrintHintText(client, "你已经给你的榴弹发射器重装弹!");
			}
			...
			if (StrEqual(ent_name, "weapon_ammo_spawn", false) && StrEqual(currentgunname, "weapon_rifle_m60", false) && GetConVarBool(M60ResupplyCVAR))
			{
				...
				PrintHintText(client, "你已经给你的 M60 重装弹!");
			}
		}
		else
		{
			if (StrEqual(ent_name, "weapon_ammo_spawn", false) && StrEqual(currentgunname, "weapon_grenade_launcher", false))
			{
				new iAmmoOffset = FindDataMapOffs(client, "m_iAmmo");
				SetEntData(client, iAmmoOffset + GRENADE_LAUNCHER_OFFSET_IAMMO, GetConVarInt(GrenadeLauncherAmmoCVAR));
				PrintHintText(client, "你已经给你的榴弹发射器重装弹!");
			...
			// later added m60 remunition code, where "gun" actually is an ammopile.
			if (StrEqual(ent_name, "weapon_ammo_spawn", false) && StrEqual(currentgunname, "weapon_rifle_m60", false))
			{
				SetEntProp(oldgun, Prop_Data, "m_iClip1", GetConVarInt(M60AmmoCVAR), 1);
				PrintHintText(client, "你已经给你的 M60 重装弹!");
			}
...
public Action:Cmd_GiveAmmo(client, args)
{
	...
	{
		PrintToChat(client, "\x01你只能给那个玩家 \x04%N\x01 弹药 \x04 如果你们有相同的武器", target);
	...
	{
		PrintToChat(client, "\x01你只能给那个玩家 \x04%N\x01 弹药 \x04", target);
	...
	{ //case: no gun this plugin recognizes
		PrintToChat(client, "Error: 那是什么狗屁玩意");
		return Plugin_Handled;
	}
	...
	{
		PrintToChat(client, "\x01他已经 \x04满\x01 弹药");
		return Plugin_Handled;
	}
	...
	if ((currentammo + donateammo) <= maxammo) //if clips ammo is less than hed need to be full
	{
		...
		PrintToChat(client, "\x01你给了 \x04%i 弹药\x01 给 \x04%N\x01", donateammo, target);
	}
	else //if given ammo exceeds his needs
	{
		...
		PrintToChat(client, "\x01你给了 \x04%i 弹药\x01 给 \x04%N\x01", neededammo, target);
	}	
	return Plugin_Handled;
}
...
public Action:Event_SpecialAmmo(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new newammo;
	new AdminId:id = GetUserAdmin(client);
	if (StrEqual(class, "upgrade_ammo_incendiary") || StrEqual(class, "upgrade_ammo_explosive"))
	{
		if(id == INVALID_ADMIN_ID || !GetAdminFlag(id, Admin_Root)) 
		{
			newammo = ammo * GetConVarInt(SpecialAmmoMultiplier);
		}
		else
		{
			newammo = ammo * GetConVarInt(AdminSpecialAmmoMultiplier);
		}
	}
	...
	PrintToChat(client, "\x01你得到了 \x04%i 个弹夹 \x01 的特殊弹药!", newammo/ammo);
}
