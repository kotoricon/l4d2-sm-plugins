...
new Float:LastUsedAirstrike[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	g_hCvarDamage =			CreateConVar(	"l4d2_airstrike_damage",		"3000",			"Hurt players by this much at the center of the explosion. Damages falls off based on the maximum distance.", CVAR_FLAGS );
	g_hCvarDistance =		CreateConVar(	"l4d2_airstrike_distance",		"200",			"The range at which the airstrike explosion can hurt players.", CVAR_FLAGS );
	g_hCvarHorde =			CreateConVar(	"l4d2_airstrike_horde",			"20",			"0=Off. The chance out of 100 to make a panic event when the bomb explodes.", CVAR_FLAGS );
	...
	cvarDelay = CreateConVar("sm_airstrike_delay","180.0","Time, in seconds, to delay the target of sm_rocket's death.", FCVAR_NONE, true, 0.0);
	...
	RegConsoleCmd("sm_kx",				CmdAirstrike);
}
...
{
	LastUsedAirstrike[client] = GetGameTime();
}
...
public Action:CmdAirstrike(client, args)
{
	...
	if(ClientPoints[client] > 15000)
	{
		if (LastUsedAirstrike[client] + GetConVarFloat(cvarDelay) > GetGameTime())
		{
			ReplyToCommand(client, "\x04[空袭]\x01 必须等待\x03 %i \x01秒后才能进行下一次空袭.", RoundFloat((LastUsedAirstrike[client] + RoundFloat(GetConVarFloat(cvarDelay))) - RoundFloat(GetGameTime())));
			return Plugin_Handled;
		}
		else
		{
...
ShowMenu(client)
{
	...
	AddMenuItem(hMenu, "1", "在鼠标指针处进行空袭");
	SetMenuTitle(hMenu, "空袭无线通信器");
...
public MainMenuHandler(Handle:menu, MenuAction:action, client, index)
{
	...
	else if( action == MenuAction_Select )
	{
		if( index == 0 )
		{
			...
			{
				CPrintToChatAll("{blue}%s{default} 呼叫了空袭,如果恶意使用(例如用于TK)请输入 {green}!report{default} 进行举报", pName);
				CloseHandle(trace);
			}
			else
			{
