...
#define MSGTAG "\x04[商店系统]\x01"
#define MSGTAG2 "\x04[商店系统]\x01 "
...
new String:AttackerID[30];
new String:SteamID[30];
new String:Query[121];
...
new boughtcost[MAXPLAYERS + 1];
new hurtcount[MAXPLAYERS + 1];
new protectcount[MAXPLAYERS + 1];
new cost[MAXPLAYERS + 1];
new tankburning[MAXPLAYERS + 1];
new tankbiled[MAXPLAYERS + 1];
new witchburning[MAXPLAYERS + 1];
new killcount[MAXPLAYERS + 1];
new headshotcount[MAXPLAYERS + 1];
new wassmoker[MAXPLAYERS + 1];
...
{
	//BuildPath(Path_SM, SavePath, 255, "data/PlayerShopData.txt");
	ConnectDB();
	...
	Notifications = CreateConVar("l4d2_points_notify", "0", "Show messages when points are earned?", FCVAR_PLUGIN);
	Enable = CreateConVar("l4d2_points_enable", "1", "Enable Point System?", FCVAR_PLUGIN);
	Modes = CreateConVar("l4d2_points_modes", "coop,survival", "Which game modes to use Point System", FCVAR_PLUGIN);
	...
	TankLimit = CreateConVar("l4d2_points_tank_limit", "16", "How many tanks to be allowed spawned per team", FCVAR_PLUGIN);
	WitchLimit = CreateConVar("l4d2_points_witch_limit", "0", "How many witchs' to be allwed spawned per team", FCVAR_PLUGIN);
	SpawnTries = CreateConVar("l4d2_points_spawn_tries", "0", "How many times to attempt respawning when buying an special infected", FCVAR_PLUGIN);
	PointsPistol = CreateConVar("l4d2_points_pistol", "10", "How many points the pistol costs", FCVAR_PLUGIN);
	PointsSMG = CreateConVar("l4d2_points_smg", "20", "How many points the smg costs", FCVAR_PLUGIN);
	PointsM16 = CreateConVar("l4d2_points_m16", "35", "How many points the m16 costs", FCVAR_PLUGIN);
	PointsHunting = CreateConVar("l4d2_points_hunting_rifle", "30", "How many points the hunting rifle costs", FCVAR_PLUGIN);
	PointsAuto = CreateConVar("l4d2_points_autoshotgun", "50", "How many points the autoshotgun costs", FCVAR_PLUGIN);
	PointsPump = CreateConVar("l4d2_points_pump", "28", "How many points the pump shotgun costs", FCVAR_PLUGIN);
	PointsGasCan = CreateConVar("l4d2_points_gascan", "300", "How many points the gas can costs", FCVAR_PLUGIN);
	PointsPropane = CreateConVar("l4d2_points_propane", "15", "How many points the propane tank costs", FCVAR_PLUGIN);
	PointsMagnum = CreateConVar("l4d2_points_magnum", "10", "How many points the magnum costs", FCVAR_PLUGIN);
	PointsSSMG = CreateConVar("l4d2_points_ssmg", "25", "How many points the silenced smg costs", FCVAR_PLUGIN);
	PointsMP5 = CreateConVar("l4d2_points_mp5", "20", "How many points the mp5 costs", FCVAR_PLUGIN);
	PointsAK = CreateConVar("l4d2_points_ak", "40", "How many points the ak47 costs", FCVAR_PLUGIN);
	PointsSCAR = CreateConVar("l4d2_points_scar", "55", "How many points the scar costs", FCVAR_PLUGIN);
	PointsSG = CreateConVar("l4d2_points_sg", "55", "How many points the sg552 costs", FCVAR_PLUGIN);
	PointsMilitary = CreateConVar("l4d2_points_military_sniper", "55", "How many points the military sniper costs", FCVAR_PLUGIN);
	PointsAWP = CreateConVar("l4d2_points_awp", "60", "How many points the awp costs", FCVAR_PLUGIN);
	PointsScout = CreateConVar("l4d2_points_scout", "30", "How many points the scout sniper costs", FCVAR_PLUGIN);
	PointsSpas = CreateConVar("l4d2_points_spas", "50", "How many points the spas shotgun costs", FCVAR_PLUGIN);
	PointsChrome = CreateConVar("l4d2_points_chrome", "32", "How many points the chrome shotgun costs", FCVAR_PLUGIN);
	PointsGL = CreateConVar("l4d2_points_grenade", "100", "How many points the grenade launcher costs", FCVAR_PLUGIN);
	PointsM60 = CreateConVar("l4d2_points_m60", "400", "How many points the m60 costs", FCVAR_PLUGIN);
	PointsOxy = CreateConVar("l4d2_points_oxygen", "10", "How many points the oxgen tank costs", FCVAR_PLUGIN);
	PointsGnome = CreateConVar("l4d2_points_gnome", "500", "How many points the gnome costs", FCVAR_PLUGIN);
	PointsCola = CreateConVar("l4d2_points_cola", "1000", "How many points cola bottles costs", FCVAR_PLUGIN);
	PointsFireWorks = CreateConVar("l4d2_points_fireworks", "15", "How many points the fireworks crate costs", FCVAR_PLUGIN);
	PointsBat = CreateConVar("l4d2_points_bat", "15", "How many points the baseball bat costs", FCVAR_PLUGIN);
	PointsMachete = CreateConVar("l4d2_points_machete", "16", "How many points the machete costs", FCVAR_PLUGIN);
	PointsKatana = CreateConVar("l4d2_points_katana", "20", "How many points the katana costs", FCVAR_PLUGIN);
	PointsKnife = CreateConVar("l4d2_points_knife", "25", "How many points the knife costs", FCVAR_PLUGIN);
	PointsShield = CreateConVar("l4d2_points_shield", "30", "How many points the shield costs", FCVAR_PLUGIN);
	PointsTonfa = CreateConVar("l4d2_points_tonfa", "28", "How many points the tonfa costs", FCVAR_PLUGIN);
	PointsFireaxe = CreateConVar("l4d2_points_fireaxe", "26", "How many points the fireaxe costs", FCVAR_PLUGIN);
	PointsGuitar = CreateConVar("l4d2_points_guitar", "24", "How many points the guitar costs", FCVAR_PLUGIN);
	PointsPan = CreateConVar("l4d2_points_pan", "29", "How many points the frying pan costs", FCVAR_PLUGIN);
	PointsCBat = CreateConVar("l4d2_points_cricketbat", "28", "How many points the cricket bat costs", FCVAR_PLUGIN);
	PointsCrow = CreateConVar("l4d2_points_crowbar", "24", "How many points the crowbar costs", FCVAR_PLUGIN);
	PointsClub = CreateConVar("l4d2_points_golfclub", "31", "How many points the golf club costs", FCVAR_PLUGIN);
	PointsSaw = CreateConVar("l4d2_points_chainsaw", "10000", "How many points the chainsaw costs", FCVAR_PLUGIN);
	PointsPipe = CreateConVar("l4d2_points_pipe", "15", "How many points the pipe bomb costs", FCVAR_PLUGIN);
	PointsMolly = CreateConVar("l4d2_points_molotov", "15", "How many points the molotov costs", FCVAR_PLUGIN);
	...
	PointsDefib = CreateConVar("l4d2_points_defib", "10", "How many points the defib costs", FCVAR_PLUGIN);
	PointsAdren = CreateConVar("l4d2_points_adrenaline", "15", "How many points the adrenaline costs", FCVAR_PLUGIN);
	PointsPills = CreateConVar("l4d2_points_pills", "10", "How many points the pills costs", FCVAR_PLUGIN);
	PointsEAmmo = CreateConVar("l4d2_points_explosive_ammo", "30", "How many points the explosive ammo costs", FCVAR_PLUGIN);
	PointsIAmmo = CreateConVar("l4d2_points_incendiary_ammo", "40", "How many points the incendiary ammo costs", FCVAR_PLUGIN);
	PointsEAmmoPack = CreateConVar("l4d2_points_explosive_ammo_pack", "80", "How many points the explosive ammo pack costs", FCVAR_PLUGIN);
	...
	STankKill = CreateConVar("l4d2_points_tankkill", "5", "How many points does killing a tank earn", FCVAR_PLUGIN);
	SWitchKill = CreateConVar("l4d2_points_witchkill", "8", "How many points does killing a witch earn", FCVAR_PLUGIN);
	...
	SHealWarning = CreateConVar("l4d2_points_heal_warning", "0", "How many points does healing a team mate who did not need healing earn", FCVAR_PLUGIN);
	SProtect = CreateConVar("l4d2_points_protect", "2", "How many points does protecting a team mate earn", FCVAR_PLUGIN);
	SRevive = CreateConVar("l4d2_points_revive", "3", "How many points does reviving a team mate earn", FCVAR_PLUGIN);
	SLedge = CreateConVar("l4d2_points_ledge", "3", "How many points does reviving a hanging team mate earn", FCVAR_PLUGIN);
	SDefib = CreateConVar("l4d2_points_defib_action", "10", "How many points does defibbing a team mate earn", FCVAR_PLUGIN);
	STBurn = CreateConVar("l4d2_points_tankburn", "3", "How many points does burning a tank earn", FCVAR_PLUGIN);
	STSolo = CreateConVar("l4d2_points_tanksolo", "10", "How many points does killing a tank single-handedly earn", FCVAR_PLUGIN);
	SWBurn = CreateConVar("l4d2_points_witchburn", "1", "How many points does burning a witch earn", FCVAR_PLUGIN);
	STag = CreateConVar("l4d2_points_bile_tank", "3", "How many points does biling a tank earn", FCVAR_PLUGIN);
	...
	PointsSuicide = CreateConVar("l4d2_points_suicide", "100", "How many points does suicide cost", FCVAR_PLUGIN);
	...
	RegConsoleCmd("sm_rebuy", Command_RBuy, "Repeat your last buy transaction");
...
public Action:Stats(Handle:timer, any:client)
{
	PrintToChatAll("\x01输入\x05 !buy \x04进入商店系统\x01");
}
...
{
	ClientPoints[GetNativeCell(1)] = GetNativeCell(2);
}	
...
public Action:Event_Kill(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	ACHECK2
	{
		...
		{
			//ClientPoints[attacker] += headpoints;
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", headpoints, AttackerID);
			SendSQLUpdate(Query);
		...
		{
			//ClientPoints[attacker] += spreepoints;
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", spreepoints, AttackerID);
			SendSQLUpdate(Query);
...
public Action:Event_Incap(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	ACHECK3
	{
		...
		//ClientPoints[attacker] += incappoints;
		Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", incappoints, AttackerID);
		SendSQLUpdate(Query);
...
public Action:Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(attacker > 0 && client > 0 && !IsFakeClient(attacker) && IsAllowedGameMode() && GetConVarInt(Enable) == 1)
	{
		...
		if(GetClientTeam(attacker) == 2)
		{
			...
			//ClientPoints[attacker] += sipoints;
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", sipoints, AttackerID);
			SendSQLUpdate(Query);
		...
		else if(GetClientTeam(attacker) == 3)
		{
			...
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", killpoints, AttackerID);
			//ClientPoints[attacker] += killpoints;
			SendSQLUpdate(Query);
...
{	
	...
	ACHECK2
	{
		...
		{
			//ClientPoints[attacker] += solopoints;
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", solopoints, AttackerID);
			SendSQLUpdate(Query);
	...
	for (new i=1; i<=MaxClients; i++)
	{
		...
		{
			//ClientPoints[i] += tankpoints;
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", tankpoints, AttackerID);
			SendSQLUpdate(Query);
...
public Action:Event_WitchDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	CCHECK2
	{
		...
		GetClientAuthString(client, AttackerID, sizeof(AttackerID));
		
		new witchpoints = GetConVarInt(SWitchKill);
		if(witchpoints <= 0) return;
		new notify = GetConVarBool(Notifications);
		//ClientPoints[client] += witchpoints;
		Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", witchpoints, AttackerID);
		SendSQLUpdate(Query);
		new crownpoints = GetConVarInt(SWitchCrown);
		if(oneshot && crownpoints > 0)
		{
			//ClientPoints[client] += crownpoints;
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", crownpoints, AttackerID);
			SendSQLUpdate(Query);
...
public Action:Event_Heal(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(subject > 0 && client > 0 && !IsFakeClient(client) && GetClientTeam(client) == 2 && IsAllowedGameMode() && GetConVarInt(Enable) == 1)
	{
		...
		if(restored > 39)
		{
			...
			//ClientPoints[client] += healpoints;
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", healpoints, AttackerID);
			SendSQLUpdate(Query);
...
public Action:Event_Protect(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(client > 0 && award == 67 && GetConVarInt(SProtect) > 0 && IsClientInGame(client) && IsClientConnected(client) && GetClientTeam(client) > 1 && !IsFakeClient(client) && IsAllowedGameMode())
	{
		...
		{
			//ClientPoints[client] += protectpoints;
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", protectpoints, AttackerID);
			SendSQLUpdate(Query);
			protectcount[client] = 0;
...
public Action:Event_Revive(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	CCHECK2
	{
		...
		{
			//ClientPoints[client] += revivepoints;
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", revivepoints, AttackerID);
			SendSQLUpdate(Query);
			if(notify) PrintToChat(client, "%s %T", MSGTAG, "Revive", LANG_SERVER, revivepoints);
		}
		else if(ledge && ledgepoints > 0)
		{
			//ClientPoints[client] += ledgepoints;
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", ledgepoints, AttackerID);
			SendSQLUpdate(Query);
...
public Action:Event_Shock(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	CCHECK2
	{
		...
		//ClientPoints[client] += defibpoints;
		Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", defibpoints, AttackerID);
		SendSQLUpdate(Query);
...
public Action:Event_Choke(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	CCHECK3
	{
		...
		//ClientPoints[client] += chokepoints;
		Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", chokepoints, AttackerID);
		SendSQLUpdate(Query);
...
public Action:Event_Boom(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(attacker > 0 && !IsFakeClient(attacker) && IsAllowedGameMode() && GetConVarInt(Enable) == 1)
	{
		...
		if(team == 3 && itag > 0)
		{
			//ClientPoints[attacker] += itag;
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", itag, AttackerID);
			SendSQLUpdate(Query);
		...
		{
			//ClientPoints[attacker] += stag;
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", stag, AttackerID);
			SendSQLUpdate(Query);
...
public Action:Event_Pounce(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	CCHECK3
	{
		...
		//ClientPoints[client] += pouncepoints;
		Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", pouncepoints, AttackerID);
		SendSQLUpdate(Query);
...
public Action:Event_Ride(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	CCHECK3
	{
		...
		//ClientPoints[client] += ridepoints;
		Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", ridepoints, AttackerID);
		SendSQLUpdate(Query);
...
public Action:Event_Carry(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	CCHECK3
	{
		...
		//ClientPoints[client] += carrypoints;
		Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", carrypoints, AttackerID);
		SendSQLUpdate(Query);
...
public Action:Event_Impact(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	CCHECK3
	{
		...
		//ClientPoints[client] += impactpoints;
		Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", impactpoints, AttackerID);
		SendSQLUpdate(Query);
...
public Action:Event_Burn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	CCHECK2
	{
		...
		{
			//ClientPoints[client] += tankpoints;
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", tankpoints, AttackerID);
			SendSQLUpdate(Query);
...
public Action:Event_Hurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(attacker > 0 && client > 0 && !IsFakeClient(attacker) && GetClientTeam(attacker) == 3 && GetClientTeam(client) == 2 && IsAllowedGameMode() && GetConVarInt(Enable) == 1 && GetConVarInt(IHurt) > 0)
	{
		...
		if( (type == 263168 || type == 265216) && hurtcount[attacker] >= 8 )
		{
			...
			//ClientPoints[attacker] += GetConVarInt(IHurt);
			Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", GetConVarInt(IHurt), AttackerID);
			SendSQLUpdate(Query);
...
public Action:ShowPoints(client,args)
{
	...
	if(IsAllowedGameMode() && GetConVarInt(Enable) == 1 && IsClientInGame(client) && IsClientConnected(client) && GetClientTeam(client) > 1 && args == 0)
	{
		ReplyToCommand(client, "%s %T", MSGTAG, "Your Points", LANG_SERVER, ClientPoints[client]);
	}
	return Plugin_Handled;
}
...
public Action:Command_RBuy(client, args)
{
	...
	else if (args == 0 && IsClientInGame(client))
	{
		...
		RemoveFlags();
		if(ClientPoints[client] < cost[client])
		{
		...
		//ClientPoints[client] -= cost[client];
		Format(Query, sizeof(Query), "UPDATE stats SET money = money - %i WHERE steamid = '%s'", cost[client], SteamID);
		SendSQLUpdate(Query);
		...
		else if(StrEqual(item[client], "give ammo", false))
		{
			...
			if(wep == -1)
			{
				...
				//ClientPoints[client] += cost[client]; //refund
				Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", cost[client], SteamID);
				SendSQLUpdate(Query);
				AddFlags();
				return Plugin_Handled;
	...
	if(args == 2)
	{
		...
		{		
			...
			for (new i = 0; i < target_count; i++)
			{
				...
				Format(Query, sizeof(Query), "UPDATE stats SET money = money + %i WHERE steamid = '%s'", amount, SteamID);
				SendSQLUpdate(Query);
				//ClientPoints[targetclient] += amount;
			}
			return Plugin_Handled;
		}	
	}
...
BuildBuyMenu(client)
{
	...
	if(GetClientTeam(client) == 2)
	{
		...
		Format(title, sizeof(title), "%T", "Points Left", LANG_SERVER, ClientPoints[client]);
	...
	switch(action)
	{
		...
		case MenuAction_Select:
		{
			...
			else if (StrEqual(choice, "yes", false))
			{
				...
				Format(Query, sizeof(Query), "SELECT money FROM stats WHERE steamid = '%s'", SteamID); 
				SQL_TQuery(db, GetClientPoints, Query, param1);
				
				if(ClientPoints[param1] < cost[param1])
				{
				...
				else
				{
					...
					//ClientPoints[param1] -= cost[param1];
					Format(Query, sizeof(Query), "UPDATE stats SET money = money - %i WHERE steamid = '%s'", cost[param1], SteamID);
					SendSQLUpdate(Query);
					RemoveFlags();
		...
		case MenuAction_Select:
		{
			...
			else if (StrEqual(choice, "yes", false))
			{
				...
				else if(StrEqual(item[param1], "give ammo", false))
				{
					...
					if(wep == -1)
					{
						if(IsClientInGame(param1)) PrintToChat(param1, "[商店系统] You must have a primary weapon to refill ammo!");
						return;
					}	
					//ClientPoints[param1] -= cost[param1];
					Format(Query, sizeof(Query), "UPDATE stats SET money = money - %i WHERE steamid = '%s'", cost[param1], SteamID);
