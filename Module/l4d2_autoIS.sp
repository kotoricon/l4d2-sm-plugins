...
public OnPluginStart()
{
	...
	hFasterResponse = CreateConVar("l4d2_ais_fast_response", "0", "[0=OFF|1=ON] Disable/Enable faster special infected response", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	hFasterSpawn = CreateConVar("l4d2_ais_fast_spawn", "0", "[0=OFF|1=ON] Disable/Enable faster special infected spawn (Enable when SI spawn rate is high)", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	hSpawnWeights[SI_BOOMER] = CreateConVar("l4d2_ais_boomer_weight", "15", "The weight for a boomer spawning", FCVAR_PLUGIN, true, 0.0);
	hSpawnWeights[SI_HUNTER] = CreateConVar("l4d2_ais_hunter_weight", "35", "The weight for a hunter spawning", FCVAR_PLUGIN, true, 0.0);
	hSpawnWeights[SI_SMOKER] = CreateConVar("l4d2_ais_smoker_weight", "25", "The weight for a smoker spawning", FCVAR_PLUGIN, true, 0.0);
	hSpawnWeights[SI_TANK] = CreateConVar("l4d2_ais_tank_weight", "-1", "[-1 = Director spawns tanks] The weight for a tank spawning", FCVAR_PLUGIN, true, -1.0);
	hSpawnWeights[SI_CHARGER] = CreateConVar("l4d2_ais_charger_weight", "15", "The weight for a charger spawning", FCVAR_PLUGIN, true, 0.0);
	hSpawnWeights[SI_JOCKEY] = CreateConVar("l4d2_ais_jockey_weight", "15", "The weight for a jockey spawning", FCVAR_PLUGIN, true, 0.0);
	hSpawnWeights[SI_SPITTER] = CreateConVar("l4d2_ais_spitter_weight", "25", "The weight for a spitter spawning", FCVAR_PLUGIN, true, 0.0);
	hSpawnLimits[SI_BOOMER] = CreateConVar("l4d2_ais_boomer_limit", "2", "The max amount of boomers present at once", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	hSpawnLimits[SI_HUNTER] = CreateConVar("l4d2_ais_hunter_limit", "4", "The max amount of hunters present at once", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	hSpawnLimits[SI_SMOKER] = CreateConVar("l4d2_ais_smoker_limit", "5", "The max amount of smokers present at once", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	...
	hSpawnLimits[SI_CHARGER] = CreateConVar("l4d2_ais_charger_limit", "3", "The max amount of chargers present at once", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	hSpawnLimits[SI_JOCKEY] = CreateConVar("l4d2_ais_jockey_limit", "3", "The max amount of jockeys present at once", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	hSpawnLimits[SI_SPITTER] = CreateConVar("l4d2_ais_spitter_limit", "2", "The max amount of spitters present at once", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	...
	hWitchLimit = CreateConVar("l4d2_ais_witch_limit", "0", "[-1 = Director spawns witches] The max amount of witches present at once (independant of l4d2_ais_limit).", FCVAR_PLUGIN, true, -1.0, true, 100.0);
	hWitchPeriod = CreateConVar("l4d2_ais_witch_period", "9999.0", "The time (seconds) interval in which exactly one witch will spawn", FCVAR_PLUGIN, true, 1.0);
	...
	hSILimit = CreateConVar("l4d2_ais_limit", "12", "The max amount of special infected at once", FCVAR_PLUGIN, true, 1.0, true, float(MAX_INFECTED));
	hSILimitMax = FindConVar("z_max_player_zombies");
	hSpawnSize = CreateConVar("l4d2_ais_spawn_size", "4", "The amount of special infected spawned at each spawn interval", FCVAR_PLUGIN, true, 1.0, true, float(MAX_INFECTED));
	hSpawnTimeMode = CreateConVar("l4d2_ais_time_mode", "1", "The spawn time mode [0=RANDOMIZED|1=INCREMENTAL|2=DECREMENTAL]", FCVAR_PLUGIN, true, 0.0, true, 2.0);
	...
	hSpawnTimeMin = CreateConVar("l4d2_ais_time_min", "20.0", "The minimum auto spawn time (seconds) for infected", FCVAR_PLUGIN, true, 0.0);
	hSpawnTimeMax = CreateConVar("l4d2_ais_time_max", "25.0", "The maximum auto spawn time (seconds) for infected", FCVAR_PLUGIN, true, 1.0);
...
public CheckPlayer()
{
	CreateTimer(5.0, SetIS, _, TIMER_REPEAT);
}

public Action:SetIS(Handle:timer)
{
	if(GetInGamePlayerCount() < 5)
	{
		SpawnSize = (GetConVarInt(hSpawnSize) / 4);
		SILimit = GetConVarInt(hSILimit) / 3;
	}
	if(GetInGamePlayerCount() < 8)
	{
		SpawnSize = (GetConVarInt(hSpawnSize) / 4);
		SILimit = GetConVarInt(hSILimit) / 2;
	}
	if(GetInGamePlayerCount() < 13)
	{
		SpawnSize = GetConVarInt(hSpawnSize) / 2;
		SILimit = GetConVarInt(hSILimit);
	}
	if(GetInGamePlayerCount() > 12)
	{
		SpawnSize = GetConVarInt(hSpawnSize);
		SILimit = GetConVarInt(hSILimit);
	}
