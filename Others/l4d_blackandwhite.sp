...
public OnPluginStart()
{
	...
	h_cvarPrintType = CreateConVar("l4d_bandw_type", "0", "0 prints to chat, 1 displays hint box.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
public EventReviveSuccess(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(GetEventBool(event, "lastlife"))
	{
		...
		{
			if(bandw_type == 1) PrintHintTextToAll("%s (%s) 已经进入濒死状态.", targetName, charName);
			else PrintToChatAll("\x05%s\x04\x01 (\x04%s\x01) 已经进入濒死状态.", targetName, charName);
		}
		//print to infected
		else if(bandw_notice == 3)
		{
			for( new x = 1; x <= GetMaxClients(); x++)
			{
				if(!IsClientInGame(x) || GetClientTeam(x) == GetClientTeam(target) || x == target || IsFakeClient(x))
					continue;
				if(bandw_type == 1) PrintHintText(x, "%s (%s) 已经进入濒死状态.", targetName, charName);
				else PrintToChat(x, "\x05%s\x04\x01 (\x04%s\x01) 已经进入濒死状态.", targetName, charName);
			}
		}
		//print to survivors
		else
