...
public OnPluginStart()
{
	...
	iMHealthMultiplier = CreateConVar("survivor_max_health_multiplier", "0", "Maximum Multiplier of Health by 5.", FCVAR_PLUGIN, true, 0.00, true, 32.00);
	...
	RegConsoleCmd("sm_armor", ActivateBuyMenu, "Check various stats on survivor.");
...
public BuyMenu(client)
{
	...
	PrintToChat(client, "\x05生命值: \x01%i/%i\n\x05护甲: \x01%i/%i\n\x05金钱: \x01$%i", iHealth, iMaxHealth, iArmor/GetConVarInt(iMArmorFactor), iMaxArmor/GetConVarInt(iMArmorFactor), iCash[client]);
	...
	if(GetClientTeam(client) == 2 && IsPlayerAlive(client))
	{
		...
		Format(buffer, sizeof(buffer), "\x05生命值: \x01%i/%i\n\x05护甲: \x01%i/%i\n\x05金钱: \x01$%i", iHealth, iMaxHealth, iArmor/GetConVarInt(iMArmorFactor), iMaxArmor/GetConVarInt(iMArmorFactor), iCash[client]);
		SetPanelTitle(BuyMenuPanel, buffer);

		new String:text[64];
		Format(text, sizeof(text), "购买护甲");
		DrawPanelText(BuyMenuPanel, text);
		
		DrawPanelItem(BuyMenuPanel, "购买");
		DrawPanelItem(BuyMenuPanel, "开关状态显示");
		DrawPanelItem(BuyMenuPanel, "关闭");
	...
	{
		PrintToChat(client, "\x01[\x03ERROR\x01] 必须在存活状态才能使用该功能.");
	}    
}
	...
	if (action == MenuAction_Select)
	{
		...
		if(param2 == 2)
		{
			if(b_ShowArmor[client] == false)
			{
				...
				PrintToChat(client, "\x04状态显示 \x01\x05开启.");
			}
			else
			{
				b_ShowArmor[client] = false;
				PrintToChat(client, "\x04状态显示 \x01\x05关闭.");
			}
		}
...
public CharacterMenu(client)
{
	if(GetClientTeam(client) == 2 && IsPlayerAlive(client))
	{
		...
		{
			AddMenuItem(menu, "option1", "生命值 (￥0) (暂时禁用)");
		}
		if(iCash[client] >= ARMOR_UPGRADE_COST)
		{
			AddMenuItem(menu, "option2", "护甲 (￥100)");
		}
		if(iCash[client] >= HEALTH_COST)
		{
			AddMenuItem(menu, "option3", "临时生命值 +25 (￥100)");
		}
		if(iCash[client] >= ARMOR_COST)
		{
			AddMenuItem(menu, "option4", "补充护甲 (￥250)");
		}
		SetMenuTitle(menu, "金钱: %d", iCash[client]);
		...
		case MenuAction_Select:
		{
			...
			if(StrEqual(item1, "option1", false))
			{
				...
				{			
					...
					new iMaxHealth = GetEntData(client, FindDataMapOffs(client, "m_iMaxHealth"), 4);
					new iHealth = GetEntData(client, FindDataMapOffs(client, "m_iHealth"), 4);
					PrintToChat(client, "\x05生命值已升级: \x01%i/%i", iHealth, iMaxHealth);
					CharacterMenu(client);
				}
				else
				{
					PrintToChat(client, "\x01已达到 \x05生命值上限\x01.");
				}
			}
			else if(StrEqual(item1, "option2", false))
			{
				...
				{			
					...
					new iMaxArmor = iMArmor[client] * GetConVarInt(iMArmorFactor) * 5;
					new iArmor = GetEntData(client, FindDataMapOffs(client, "m_ArmorValue"), 4);
					PrintToChat(client, "\x05护甲已升级: \x01%i/%i", iArmor/GetConVarInt(iMArmorFactor), iMaxArmor/GetConVarInt(iMArmorFactor));
					CharacterMenu(client);
				}
				else
				{
					PrintToChat(client, "\x01已达到 \x05护甲值\x01.");
				}
			}
			else if(StrEqual(item1, "option3", false))
			{
				...
				if(iHealthDif >= 25)
				{
					...
					PrintToChat(client, "\x05 临时生命值已恢复 25 (黑白状态无帮助)");
					CharacterMenu(client);
				}
				else
				{
					PrintToChat(client, "\x01已是 \x05最大生命值状态\x01.");
				}
			}
			else if(StrEqual(item1, "option4", false))
			{
				...
				if(iArmorDif >= 25)
				{
					...
					PrintToChat(client, "\x05 护甲已恢复 25");
					CharacterMenu(client);
				}
				else
				{
					PrintToChat(client, "\x01已是 \x05最大护甲值状态\x01.");
				}
			}
		}
	}
...
public event_PlayerHurt(Handle:event, const String:name[], bool:Broadcast) 
{
	...
	if(GetClientTeam(client) == 2)
	{
		...
		if(b_ShowArmor[client] == true)
		{
			...
			PrintHintText(client, "\x05生命值: %i/%i\n护甲: %i/%i", iHealth, iMaxHealth, iArmor/GetConVarInt(iMArmorFactor), iMaxArmor/GetConVarInt(iMArmorFactor));
		}
	}
}
...
public Action:event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(StrEqual(VictimName, "Smoker", false))
	{
		...
		if(b_ShowArmor[client] == true)
		{
			PrintHintText(client, "你拾取了 ￥%d", CASH_SMOKER);
		}
	}
	else if(StrEqual(VictimName, "Boomer", false))
	{
		...
		if(b_ShowArmor[client] == true)
		{
			PrintHintText(client, "你拾取了 ￥%d", CASH_BOOMER);
		}
	}
	else if(StrEqual(VictimName, "Hunter", false))
	{
		...
		if(b_ShowArmor[client] == true)
		{
			PrintHintText(client, "你拾取了 ￥%d", CASH_HUNTER);
		}
	}
	else if(StrEqual(VictimName, "Witch", false))
	{
		...
		if(b_ShowArmor[client] == true)
		{
			PrintHintText(client, "你拾取了 ￥%d", CASH_WITCH);
		}
	}
	else if(StrEqual(VictimName, "Tank", false))
	{
		...
		if(b_ShowArmor[client] == true)
		{
			PrintHintText(client, "你拾取了 ￥%d", CASH_TANK);
		}
	}
}
...
public Action:event_InfectedDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(GetCashProbability < GetConVarFloat(iCashProbability) * 100)
	{
		...
		if(b_ShowArmor[client] == true)
		{
			PrintHintText(client, "你获得了 ￥%d", CashValue);
		}
	}
}
...
ClientSaveToFileSave(targetid)
{
	...
	KvSetNum(kv, "最大生命值", iMHealth[targetid]);
	KvSetNum(kv, "最大护甲值", iMArmor[targetid]);
	KvSetNum(kv, "金钱", iCash[targetid]);
	KvRewind(kv);
...
ClientSaveToFileLoad(targetid)
{
	decl Handle:kv;
	decl String:cName[MAX_NAME_LENGTH];
	kv = CreateKeyValues("Armor-System Save");
	FileToKeyValues(kv, SavePath);

	GetClientAuthString(targetid, cName, sizeof(cName));

	KvJumpToKey(kv, cName, true);
	iMHealth[targetid] = KvGetNum(kv, "最大生命值", iMHealth[targetid]);
	iMArmor[targetid] = KvGetNum(kv, "最大护甲值", iMArmor[targetid]);
	iCash[targetid] = KvGetNum(kv, "金钱", 0);
	CloseHandle(kv);
}
